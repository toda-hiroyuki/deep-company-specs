...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password.length < 8) {
      setError(t("guest.auth.passwordTooShort"));
      return;
    }
    if (password !== confirmPassword) {
      setError(t("guest.auth.passwordMismatch"));
      return;
    }
    ...
    try {
      const res = await fetch("/api/v1/guest/auth/register", {
        method: "POST",
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {t("guest.auth.passwordTooShort")}
              </p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {t("guest.auth.confirmPassword")}
              </label>
              <input
                type="password"
            ...
            >
              {loading ? t("loading") : t("guest.auth.register")}
            </button>
          </form>

          {/* Login link */}
          <p className="text-center text-sm text-gray-600 mt-6">
            {t("guest.auth.hasAccount")}{" "}
            <Link
              href="/auth/login"
